...
const sandbox = AdaptiveInspectionSessionSandbox.createSandboxSession(draft, {
    sandboxId: "sandbox-human-work-layer-integration-001"
});
...
assert.ok(workView.currentModule.moduleIndex >= 1);
assert.ok(workView.currentModule.chapterNumber);
assert.ok(workView.currentModule.chapterTitle);
assert.ok(workView.currentModule.questionCount >= 1);
...
console.log("InspectionHumanWorkLayer catalog integration test passed");
...
console.log(`Question text: ${workView.currentQuestion.questionText}`);
