...
import InspectionDecisionDomainModel, * as InspectionDecisionModule from "../portal/core/InspectionDecisionDomainModel.js";

const EXPECTED_ERROR = "InspectionDecisionDomainModel: invalid Inspection Decision.";
const VALID_INPUT = Object.freeze({
    decisionId: "DEC-1",
    recommendationId: "REC-1",
    decision: "Proceed with the recommended specialist inspection.",
...
function rejectsCreate(value) {
    assert.throws(
        () => InspectionDecisionDomainModel.createDecision(value),
...
function rejectsValidate(value) {
    assert.throws(
        () => InspectionDecisionDomainModel.validateDecision(value),
        (error) => error instanceof Error && error.message === EXPECTED_ERROR
    );
}
...
test("exports only the governed default public contract", () => {
    assert.deepEqual(Object.keys(InspectionDecisionModule), ["default"]);
    assert.equal(InspectionDecisionModule.default, InspectionDecisionDomainModel);
    const publicMembers = Object.getOwnPropertyNames(InspectionDecisionDomainModel)
        .filter((name) => !["length", "name", "prototype"].includes(name));
    assert.deepEqual(publicMembers, ["VERSION", "createDecision", "validateDecision"]);
});

test("VERSION is exactly 1.0", () => {
    assert.equal(InspectionDecisionDomainModel.VERSION, "1.0");
});

test("creates the exact canonical decision in canonical field order", () => {
    const result = InspectionDecisionDomainModel.createDecision(VALID_INPUT);
...
test("sets version internally and rejects version in creation input", () => {
    assert.equal(InspectionDecisionDomainModel.createDecision(VALID_INPUT).version, "1.0");
...
test("rejects all excluded architectural and unknown fields", () => {
    [
        "id", "assessmentId", "assessmentIds", "findingId", "findingIds",
        "evidenceId", "evidenceIds", "observationId", "caseId", "sessionId",
        "areaId", "recommendation", "status", "state", "approved", "rejected",
        "accepted", "workflow", "reviewer", "approver", "authorization", "risk",
        "riskLevel", "riskScore", "priority", "severity", "confidence", "rationale",
        "reasoning", "summary", "persisted", "metadata", "createdBy", "updatedBy",
        "updatedAt", "unknownField"
...
test("accepts ordinary and null-prototype roots", () => {
    assert.deepEqual(InspectionDecisionDomainModel.createDecision(VALID_INPUT), {
        version: "1.0",
        ...VALID_INPUT
    });
    const input = Object.assign(Object.create(null), VALID_INPUT);
    assert.deepEqual(InspectionDecisionDomainModel.createDecision(input), {
        version: "1.0",
        ...VALID_INPUT
    });
});
...
test("string fields preserve unicode whitespace and decision line breaks", () => {
    const input = {
        decisionId: "  DEC-Ä-一  ",
        recommendationId: "  REC-é-二  ",
        decision: "  Menschliche Entscheidung – unverändert.\n第二行  ",
        createdAt: VALID_INPUT.createdAt
    };
    const result = InspectionDecisionDomainModel.createDecision(input);
...
test("all string fields reject the complete invalid type matrix", () => {
    ...
    ["decisionId", "recommendationId", "decision"].forEach((field) => {
        invalidValues.forEach((value) => rejectsCreate({ ...VALID_INPUT, [field]: value }));
    });
});

test("does not resolve the parent recommendation", () => {
    const result = InspectionDecisionDomainModel.createDecision({
        ...VALID_INPUT,
        recommendationId: "NONEXISTENT-RECOMMENDATION"
    });
    assert.equal(result.recommendationId, "NONEXISTENT-RECOMMENDATION");
    ["assessmentId", "findingId", "evidenceIds", "recommendation"]
...
test("accepts canonical timestamp calendar boundaries including year zero", () => {
    [
        ...
        assert.equal(
            InspectionDecisionDomainModel.createDecision({ ...VALID_INPUT, createdAt }).createdAt,
            createdAt
        );
    });
});
...
test("captures createDecision input exactly once", () => {
    const { proxy, counts } = instrumentRecord({ ...VALID_INPUT });
    assert.deepEqual(InspectionDecisionDomainModel.createDecision(proxy), {
        version: "1.0",
        ...VALID_INPUT
    });
...
test("captures validateDecision input exactly once", () => {
    const canonical = { version: "1.0", ...VALID_INPUT };
    const { proxy, counts } = instrumentRecord(canonical);
    assert.deepEqual(
        InspectionDecisionDomainModel.validateDecision(proxy),
        { valid: true, errors: [] }
    );
...
test("rejects descriptor instability without executing accessors", () => {
    let invalidateRecommendationId = false;
    const proxy = new Proxy({ ...VALID_INPUT }, {
        getOwnPropertyDescriptor(target, key) {
            if (key === "decisionId") {
                invalidateRecommendationId = true;
                return Reflect.getOwnPropertyDescriptor(target, key);
            }
            if (key === "recommendationId" && invalidateRecommendationId) {
                return {
                    configurable: true,
                    enumerable: true,
                    get() {
                        return target.recommendationId;
                    }
                };
            }
            return Reflect.getOwnPropertyDescriptor(target, key);
...
test("is deterministic while returning separate root objects", () => {
    const first = InspectionDecisionDomainModel.createDecision(VALID_INPUT);
    const second = InspectionDecisionDomainModel.createDecision(VALID_INPUT);
...
test("validates only the complete canonical contract with frozen success output", () => {
    const canonical = { version: "1.0", ...VALID_INPUT };
    const result = InspectionDecisionDomainModel.validateDecision(canonical);
...
test("production source has no forbidden dependencies or mechanisms", () => {
    const source = fs.readFileSync(
        new URL("../portal/core/InspectionDecisionDomainModel.js", import.meta.url),
        "utf8"
    );
    const forbidden = [
        ...
        "RecommendationManager", "RecommendationGenerationEngine",
        ...
        "Report", "Export", "ExpertIntelligence", "KnowledgeProvider",
...
console.log(`Inspection Decision Domain Model: ${passed}/${passed} groups passed.`);
